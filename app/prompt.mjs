/**
 * 원문을 카드 대본(script.json)으로 바꾸라는 지시문을 만든다.
 *
 * 이 지시문은 CARD_RULES.md 의 요약판이다.
 * UI에서 "대본 만들기"를 누르면 이 지시문이 Claude Code CLI 로 전달되고,
 * CLI 가 없는 환경에서는 사용자가 복사해서 클로드에 직접 붙여넣는다.
 */
/**
 * 1번 카드 훅 문구를 어떤 형태로 뽑을지 — `config/style.json` 의 `hook.style`.
 *
 * 쇼츠는 **첫 3초**에서 절반이 나간다. 그 3초에 들어가는 문장이 질문형이면
 * ("적금 2.8%가 아쉽다면?") 시청자는 답을 기다려야 하는데, 기다릴 이유가 없어서 넘긴다.
 * 결론이나 숫자를 먼저 깔면 "그게 진짜야?" 를 확인하려고 남는다.
 *
 * 그래서 훅을 **한 가지로 고정하지 않고** 주제에 맞춰 고를 수 있게 했다.
 */
const HOOK_STYLES = {
  'number-first': {
    name: '결론·숫자 선공개',
    rule: `**가장 센 숫자나 결론을 1번 카드에서 바로 깐다.** 아껴두지 마라.
  숫자를 먼저 보여주면 "그게 진짜인가" 를 확인하려고 남는다.`,
    good: [
      '"1위는 1억을\\n넘깁니다"',
      '"전국 대학 등록금 TOP 5\\n1위에 1,000만 원"',
      '"20위도 839만 원\\n입니다"',
    ],
  },
  comparison: {
    name: '직관적 비교',
    rule: `**숫자를 아는 크기로 바꿔서 던진다.** 만 원 단위는 감이 안 오지만
  "중형차 한 대 값" 은 바로 와닿는다. 원문에 있는 수치로만 환산해라.`,
    good: [
      '"4년이면\\n중형차 한 대 값"',
      '"한 달 월급을\\n통째로 냅니다"',
      '"1위와 20위가\\n3배 차이납니다"',
    ],
  },
  provocative: {
    name: '손해·오해 찌르기',
    rule: `**"이대로 두면 손해" 를 건드린다.** 단, 사실에서 벗어나면 안 된다.
  원문에 없는 위협을 지어내지 마라 — 과장 훅은 조회수보다 신뢰를 먼저 깎는다.`,
    good: [
      '"은행 적금에 묶어두면\\n손해 보는 이유"',
      '"순위만 믿고 고르면\\n틀립니다"',
      '"대기업보다\\n높은 곳이 있습니다"',
    ],
  },
  question: {
    name: '질문형 (예전 방식)',
    rule: '궁금하게 만드는 질문을 던진다. 답을 1번 카드에서 주지 않는다.',
    good: ['"적금 2.8%가\\n아쉽다면?"', '"1위는 어디일까"'],
  },
};

export const buildPrompt = ({ text, source, accent, cardCount = 7, hookStyle = 'number-first' }) => {
  const hook = HOOK_STYLES[hookStyle] ?? HOOK_STYLES['number-first'];
  return `
너는 "해설형 정보 쇼츠" 대본 작가다. 아래 원문을 읽고 카드 대본 JSON을 만들어라.

# 절대 규칙
- **원문 문장을 그대로 옮기지 마라.** 사실과 숫자만 가져와 처음부터 다시 써라.
- **원문에 없는 수치나 사실을 지어내지 마라.** 원문이 애매하면 애매한 채로 줄여라.
- 출력은 **JSON 하나만**. 설명, 인사말, 마크다운 코드펜스 바깥의 어떤 텍스트도 쓰지 마라.

# 0단계 — 원문에서 "핵심이 무엇인가"부터 정해라 (가장 중요)

카드를 쓰기 전에 먼저 판단해라. **원문의 핵심이 목록·순위·표 그 자체인 경우가 있다.**
그럴 때 상위 몇 개만 뽑고 나머지를 설명으로 때우면 영상이 죽는다.
"TOP 20"이 원문이면 시청자가 보러 온 것은 **20개 전부**다.

- 목록·순위가 핵심이면 → **전부 실어라.** 한 카드에 안 들어가면 카드를 나눠서 이어 보여준다.
- 설명·제도가 핵심이면 → 숫자와 조건을 도표로 정리한다.
- **원문에 있는 핵심 수치를 빠뜨리지 마라.** 줄여야 한다면 설명을 줄이고 데이터를 남겨라.

# 1단계 — 구성(format)을 골라라

주제마다 똑같은 뼈대를 쓰면 영상이 전부 비슷해지고 지루해진다.
아래 다섯 중 원문에 맞는 것을 **하나 고르고**, 그 구성대로 ${cardCount}장을 짜라.

**A. ranking — 순위·TOP N·랭킹·"가장 ~한 곳"**
| # | type | 내용 |
|---|---|---|
| 1 | hook | headline 에 "○○ TOP N". **ranklist + revealCount:1** — 1위만 공개하고 나머지는 가린다 (아래 "1번 카드" 참고) |
| 2~4 | fact | **ranklist 로 순위를 나눠서 전부** (한 장에 최대 14행, 보통 7~10행이 보기 좋다) |
| 5 | example | 그 숫자를 내 상황으로 환산한 **직접 계산** (월 실수령, 내 연봉과의 차이 등) |
| 6 | caveat | 이 순위를 그대로 믿으면 안 되는 이유 (평균의 함정, 집계 기준, 표본) |
| ${cardCount} | conclusion | 결론 + cta |

**B. explainer — 제도·혜택·지원금 설명** (기본형)
1 hook(stat) → 2 fact(핵심 사실) → 3 condition(checklist 대상 조건) →
4 example(calc 직접 계산) → 5 fact(flow 절차) → 6 caveat → ${cardCount} conclusion

**C. howto — 신청 방법·절차가 핵심**
1 hook → 2 condition(체크리스트) → 3~5 fact(flow/timeline 으로 단계) →
example 1장 + caveat 1장 반드시 포함 → ${cardCount} conclusion

**D. compare — A vs B 중 뭘 고를까**
1 hook → 2~3 fact(table 비교표, 행을 아끼지 말고 실제로 다른 항목을 다 넣어라) →
4 example(내 조건에 넣어 계산) → 5 caveat → ${cardCount} conclusion

**E. change — 변화·추이·전망**
1 hook(stat) → 2~3 fact(trend/bar) → 4 example → 5 caveat(전망의 한계) → ${cardCount} conclusion

## 어떤 구성이든 지켜야 하는 것
- 1번은 반드시 type "hook", 마지막은 반드시 type "conclusion".
- **1번 카드에는 headline 을 반드시 넣는다** (아래 headline 항목 참고).
- **example(직접 계산) 1장과 caveat(주의점) 1장은 어떤 구성에서도 반드시 들어간다.**
- origin이 "creator"인 카드가 3장 이상이어야 한다.
- 훅에 "오늘은 ~에 대해 알아보겠습니다" 같은 도입부를 쓰지 마라. 원문 제목을 베끼지 마라.
- 카드 수가 부족해서 데이터가 잘린다면 ${cardCount}장을 넘겨도 된다. **데이터가 우선이다.**

# 1번 카드 (훅) — 첫 0.5초에 다 끝난다

쇼츠는 **첫 0.5초**에 계속 볼지 말지가 정해진다. 그 시간에 읽히는 건 많아야 두세 덩어리다.
그래서 1번 카드는 **덜어내는 게 핵심**이다. 예전에는 글자 덩어리가 9개나 들어가서
정작 "무슨 영상인지"가 묻혔다.

**1번 카드에 넣는 것은 이 세 개뿐이다.**

1. \`headline\` — 주제. 화면에서 **가장 큰 글자**로 그려진다
2. \`title\` — 한 줄짜리 후킹 문장 (배너보다 작게 그려진다)
3. \`visual\` — **증거**. 순위 영상이면 ranklist 로 1위만 공개하고 나머지는 가린다

**넣지 마라:** \`kicker\`, \`body\`. 1번 카드에서는 **그려지지도 않는다.**

## 순위 영상의 1번 카드 — 1위만 공개하고 나머지는 가린다

\`\`\`json
{"type":"hook","origin":"creator",
 "headline":"공기업 평균연봉 TOP 16",
 "title":"2위부터가\\n더 놀랍습니다",
 "visual":{"kind":"ranklist","totalRanks":16,"revealCount":1,"items":[
   {"rank":1,"label":"한국수력원자력","value":"1억 847만 원","highlight":true},
   {"rank":2,"label":"한국가스공사","value":"1억 609만 원"},
   {"rank":3,"label":"한국중부발전","value":"1억 233만 원"}]}}
\`\`\`

- \`revealCount:1\` 이면 **1위만 선명하고 2위부터는 흐릿하게 가려진다.**
  순위 숫자는 안 가려지므로, 몇 개가 더 남았는지 보여서 기다리게 된다.
- 가려질 행도 **값을 제대로 채워라.** 2번 카드부터 그대로 다시 쓰이고,
  1번 카드에서도 흐릿한 글자 길이로 "진짜 데이터"라는 게 보인다.
- 항목은 **5~7행**이 좋다. 너무 적으면 화면 아래가 비고, 너무 많으면 글자가 작아진다.
- ⚠️ **title 에 1위 값을 되풀이하지 마라.** 표에 이미 크게 나와 있다.
  같은 숫자를 두 번 말하면 읽을 건 늘고 아는 건 안 는다.
  - 나쁜 예: "1위는 1억을\\n넘깁니다"  ← 표가 이미 말하고 있다
  - 좋은 예: "2위부터가\\n더 놀랍습니다" / "16위도\\n8천만 원입니다"
  - **가려진 쪽을 가리켜라.** 그게 계속 보는 이유가 된다.

순위 영상이 아니면 1번 카드 visual 은 \`stat\`(핵심 숫자 하나)을 쓴다.
이때도 title 이 그 숫자를 되풀이하지 않게 하라.

## 1번 카드 내레이션은 짧게

1번 카드가 길면 **정지 화면을 5초 보게 된다.** 가장 큰 이탈 요인이다.
1번 카드 \`narration\` 은 **한 문장, 15자 이내**로 써라 (예: "1위는 1억이 넘습니다").
나머지 카드는 평소대로 한두 문장.

# headline — 1번 카드에만, **반드시** 넣어라 (가장 중요)

첫 화면에서 **"이게 무슨 영상인지"** 를 모르면 시청자는 그냥 넘긴다.
순위 영상인데 무슨 순위인지 안 보이면 볼 이유가 없다.
첫 프레임은 썸네일로도 쓰이므로, 여기서 주제가 읽혀야 한다.

1번 카드에 \`headline\` 을 넣어라. accent 색 배너로 크게 박힌다.

- **주제 + 범위(개수·기간·대상)가 다 들어간 정확한 한 줄.** 8~20자.
- 원문을 보지 않은 사람이 읽고 무슨 영상인지 바로 알아야 한다.
- 한 줄로 끝나야 한다. 길면 배너가 접혀서 제목처럼 안 보인다.

| 좋은 예 | 나쁜 예 | 왜 |
|---|---|---|
| 한국 부자 TOP 20 | 부자들 이야기 | 범위가 없다 |
| 공기업 평균연봉 TOP 16 | 연봉 순위 | 무슨 연봉인지 모른다 |
| 2026 청년월세 지원금 | 알아두면 좋은 정보 | 주제가 없다 |
| 서울 아파트값 5년 변화 | 부동산 근황 | 대상·기간이 없다 |

# title
- 최대 3줄, 한 줄 15~17자. 줄바꿈은 \\n 으로 직접 지정하고 의미 단위로 끊어라.
- 마침표를 쓰지 마라. 물음표·느낌표는 훅에서만.
- 도표가 있는 카드는 제목이 작게 렌더링되므로, 도표가 무슨 말을 하는지 알려주는 한 줄이면 충분하다.
- **1번 카드의 title 은 headline 을 되풀이하지 말고 "후킹 문장"을 써라.**
  headline 이 "무엇"을 말했으니, title 은 **끝까지 보게 만드는 한 방**을 담는다.

  이 영상의 훅 방식: **${hook.name}**
  ${hook.rule}

  이렇게 써라:
${hook.good.map((g) => `  - ${g}`).join('\n')}

  - 쓰지 마라: "알아볼까요", "정리해봤습니다", "~에 대해 알아보겠습니다"
  - 숫자가 들어간 덩어리("1,128만 원", "8.25%", "TOP 20")는 화면에서 **자동으로 크게·테마색으로**
    강조된다. 그러니 훅 문장에 **숫자를 최소 하나는 넣어라.** 숫자가 없으면 밋밋하게 지나간다.

# narration (모든 카드 필수)
- 제작자가 직접 읽을 구어체 문장. 한두 문장.
- 자막을 그대로 읽지 말고, 화면에 없는 설명을 더해라.

# visual (도표) — 카드마다 붙여라. 글자만 있는 카드는 1장 이하.

**항목 수 제한은 없다.** 도표는 항목 수에 맞춰 글자·행 높이가 자동으로 줄어든다.
다만 한 카드에 읽을 수 있는 한계가 있으니, 넘치면 **줄이지 말고 카드를 나눠라.**

| 도표 | 한 카드 최대 | 보기 좋은 수 |
|---|---|---|
| ranklist | 14행 | 7~10행 |
| bar | 8개 | 3~5개 |
| checklist | 10개 | 3~5개 |
| table | 10행 | 3~5행 |
| calc | 12줄 | 3~6줄 |
| timeline | 8개 | 3~5개 |
| flow(세로) | 7개 | 3~4개 |
| donut | 4조각 | 2~3조각 |
| trend | 8점 | 4~6점 |

글자가 길어 두 줄로 접히는 항목(체크리스트·비교표·타임라인)은 위 최대의 절반쯤으로 잡아라.

한 영상 안에서 도표 종류를 섞어라. 같은 종류가 연달아 3장 나오면 지루하다
(단, ranklist 로 순위를 이어 보여주는 것은 예외 — 그게 콘텐츠다).

- ranklist: **순위 목록 / TOP N / 랭킹.** 순위 원문이면 이걸 써서 전부 보여줘라.

  ⚠️ **모든 행에 "value"(오른쪽에 찍히는 수치)를 반드시 넣어라. 예외 없다.**
  순위와 이름만 있으면 그냥 이름 목록이라 볼 이유가 없다. 화면 오른쪽에 값을 찍을
  자리가 이미 잡혀 있어서, 비워두면 **빈칸이 그대로 나간다.**
  (비어 있으면 npm run check 가 막는다)

  - 좋은 예: {"rank":1,"label":"서울대","value":"4,109천㎡"}
  - 좋은 예: {"rank":1,"label":"한국수력원자력","value":"1억 847만 원"}
  - **나쁜 예**: {"rank":1,"label":"서울대"}              ← 값이 없다
  - **나쁜 예**: {"rank":1,"label":"서울대","value":"1위"} ← 순위를 되풀이할 뿐이다

  원문에 숫자가 없는 순위라면 **짧은 설명이라도** 넣어라 ("3년 연속", "유일한 국립").
  단위가 모든 행에 같으면 "unit" 에 빼서 적어도 된다.

  "note" 는 이름 밑에 작게 붙는 한마디(12자 이내, 선택) — "관악캠퍼스 포함" 처럼
  그 행만의 단서가 있을 때만 쓴다. 없으면 넣지 마라.

  "barValue" 는 막대 길이용 숫자인데 **보통 안 적어도 된다** — "value" 에서 알아서
  읽는다("1억 847만 원" 도 제대로 읽는다). 값이 "상", "중" 처럼 숫자가 아닐 때만 적어라.

  나눠 실을 땐 rank 를 직접 지정하고, totalRanks 에 전체 개수를 넣어라.
  highlight 는 한 카드에 하나만 (없어도 된다).
  **끝까지 보게 하려면 낮은 순위부터 거꾸로 올라가는 구성도 좋다**
  (20~14위 → 13~7위 → 6~1위). 그러면 1위가 마지막에 나온다.
  {"kind":"ranklist","totalRanks":20,"items":[
    {"rank":1,"label":"한국수력원자력","value":"1억 847만 원","highlight":true},
    {"rank":2,"label":"한국가스공사","value":"1억 609만 원"}]}
- stat: 핵심 숫자 하나
  {"kind":"stat","value":"23,000","unit":"원","caption":"어떻게 나온 숫자인지","delta":{"text":"작년 대비 +12%","dir":"up"}}
- bar: 항목별 크기 비교
  {"kind":"bar","unit":"원","items":[{"label":"A","value":12400,"highlight":true},{"label":"B","value":6850,"note":"보조설명"}]}
- trend: 시간에 따른 변화
  {"kind":"trend","unit":"억","points":[{"label":"2021","value":820},{"label":"2025","value":1180}]}
- donut: 전체 중 비중
  {"kind":"donut","centerLabel":"100%","slices":[{"label":"소멸","value":62,"highlight":true},{"label":"전환","value":38}]}
- flow: 관계 도표 / 단계 / 인과관계
  {"kind":"flow","direction":"down","nodes":[{"label":"본인 인증","note":"휴대폰"},{"label":"입금","highlight":true}]}
- calc: 직접 계산한 내역
  {"kind":"calc","lines":[{"label":"A카드","value":"12,400원"}],"result":{"label":"합계","value":"22,370원"}}
- checklist: 해당/비해당
  {"kind":"checklist","items":[{"text":"3년 넘게 쓴 카드가 있다","ok":true},{"text":"작년에 정리했다","ok":false}]}
- table: 2열 비교표
  {"kind":"table","headers":["A","B"],"highlightCol":1,"rows":[{"label":"기한","a":"5년","b":"1년"}]}
- timeline: 시점별 변화
  {"kind":"timeline","items":[{"when":"지금","label":"조회","highlight":true},{"when":"연말","label":"소멸"}]}

카드 역할별로 잘 맞는 도표: hook→stat / fact→ranklist·table·flow·trend / condition→checklist /
example→calc·bar / caveat→checklist(ok:false 위주) / conclusion→timeline·stat

# durationSec
narration 공백 제외 글자 수 ÷ 5.2 + 0.7, 최소 2.5. 소수점 첫째 자리.

# publish — 업로드용 문구 (반드시 포함)
영상과 함께 각 플랫폼에 올릴 문구를 같이 만들어라.
- youtube.title: 60자 이내. 검색될 만한 핵심어를 앞에 두되 낚시성 과장은 쓰지 마라.
- youtube.description: 3~5줄. 첫 줄은 영상 핵심 한 문장, 그 다음 카드 내용 요약,
  마지막에 출처(매체·제목). #쇼츠 같은 태그 2~3개를 맨 끝에.
- youtube.tags: 5~8개 문자열 배열. # 없이 단어만.
- instagram.caption: 첫 줄이 훅. 3~4줄. 끝에 **해시태그를 정확히 5개만** 줄바꿈 후 한 줄로.
  (5개보다 많아도 적어도 안 된다)
- threads.text: **말투가 다르다. 반말로, 친구한테 알려주듯 친근하게 써라.**
  · 공백 포함 180~210자
  · 첫 줄은 궁금하게 만드는 한 줄 (끝에 이모지 하나)
  · "~더라", "~했거든", "~해봐" 같은 구어체를 쓴다. 존댓말과 섞지 마라
  · 자기 경험처럼 자연스럽게 ("나도 해봤는데", "찾아보니까")
  · 마지막은 댓글·저장 유도 한 줄 (이모지 하나)
  · 이모지는 전체 2개 정도만. 남발하지 마라
  · 쓰레드는 해시태그를 하나만 지원한다. 주제 태그 1개만 맨 끝에 붙이거나 생략해라
  예시 톤: "이거 모르는 사람 은근 많더라 👀 나도 어제 찾아보고 알았는데 …
  3분이면 되니까 오늘 한 번 해봐! 궁금한 거 있으면 댓글 달아줘 🙌"
모든 문구에 원문 문장을 그대로 옮기지 마라.
**업로드 문구에는 확인 기준일(날짜)을 넣지 마라.** 세 플랫폼 문구 모두 해당한다.
영상 마지막 카드에 이미 나오고, 문구에 날짜가 박히면 다시 올릴 때 고쳐야 한다.

# 출력 형식 (이 구조 그대로)
{
  "topic": "파일명에 쓸 짧은 주제 (10자 이내)",
  "publish": {
    "youtube": { "title": "...", "description": "...", "tags": ["...", "..."] },
    "instagram": { "caption": "..." },
    "threads": { "text": "..." }
  },
  "accent": "${accent}",
  "source": ${JSON.stringify(source)},
  "cards": [
    {
      "type": "hook",
      "origin": "creator",
      "kicker": "상단 라벨 8자 이내 (선택)",
      "headline": "한국 부자 TOP 20  ← 1번 카드에만, 필수",
      "title": "후킹 문장\\n두 번째 줄",
      "body": "보조 설명 한 줄 (선택)",
      "narration": "직접 읽을 문장",
      "durationSec": 6.1,
      "visual": { "kind": "stat", "value": "...", "unit": "..." }
    }
  ]
}
마지막 카드에만 "cta": "짧은 행동 유도 한 줄" 을 넣어라.

# 원문
---
${text}
---

JSON만 출력하라.
`.trim();
};
