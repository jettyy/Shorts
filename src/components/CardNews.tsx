 ...
 * 겹치는 구간에서 두 카드의 글자가 포개지지 않도록 이동 거리를 크게 잡았다 → Card.tsx
 */
 ...
 * 카드 개수/텍스트/타이밍만 바뀌면 그대로 새 영상이 되는 재사용 템플릿.
 */
  ...
  return (
    <AbsoluteFill>
      ...
      {cards.map((card, i) => {
        ...
        return (
          <Sequence key={i} from={from} durationInFrames={length} name={`카드 ${i + 1}`}>
            <Card card={card} index={card.index ?? i + 1} total={cards.length} />
          </Sequence>
        );
      })}
