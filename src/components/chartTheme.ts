 ...
 *    테마마다 배경색이 달라진 뒤에도 다시 쟀다. 가장 밝은 배경 단계(gold #122350,
 *    mint #052B28) 기준으로도 SERIES 최저 대비가 3.9:1 이라, 큰 그래픽 요소 기준(3:1)을 넘는다.
 *    새 테마를 넣을 때는 배경 중간 단계를 이보다 밝게 잡지 않는다.
 ...
 * 배경이 테마마다 다르므로 실제 값은 `accent.surface` 에서 온다.
 * 여기 있는 값은 accent 를 못 받는 자리를 위한 기본값(gold 기준)이다.
 */
