...
export type RankListVisual = {
  ...
  items: {
    ...
    label: string;
    /**
     * 화면 **오른쪽에 그대로 찍히는 값** (예: "1억 847만 원", "4,109천㎡").
     *
     * 순위와 이름만 있고 값이 없으면 그냥 목록이라 볼 이유가 없다.
     * 숫자가 없는 순위라면 짧은 설명이라도 넣는다 (예: "3년 연속").
     * **비워두면 `npm run check` 가 막는다.**
     */
    value: string;
    /** 이름 밑에 작게 붙는 한마디 (선택, 12자 이내) — "관악캠퍼스 포함" 같은 단서 */
    note?: string;
    /**
     * 막대 길이용 숫자값 (선택).
     * 안 적으면 `value` 에서 읽어낸다 — "1억 847만 원" 도 제대로 읽는다(`lib/amount.ts`).
     */
    barValue?: number;
  ...
  totalRanks?: number;
  /**
   * **몇 등까지 선명하게 보여줄지** (선택). 나머지 행은 흐릿하게 가려진다.
   *
   * 1번 카드에서 쓴다. 주제 배너 + 1위만 공개하고 나머지를 가려두면
   * **증거(진짜 순위가 있다)와 궁금증(나머지는 뭐지)을 첫 화면에 같이** 담을 수 있다.
   * 순위 숫자는 가리지 않는다 — 몇 개가 더 남았는지 보여야 기다린다.
   *
   * 생략하면 전부 선명하게 나온다(2번 카드부터는 생략한다).
   */
  revealCount?: number;
  /**
...
export type Card = {
  ...
  kicker?: string;
  /**
   * **이 영상이 무슨 영상인지** 한 줄로 말하는 제목. 1번 카드에만 넣는다.
   *
   * 쇼츠는 첫 프레임에서 "이게 뭐에 대한 영상인지" 바로 알려주지 않으면 그냥 넘어간다.
   * 순위 영상인데 무슨 순위인지 안 보이면 볼 이유가 없다.
   * 그래서 accent 색 배너로 눈에 띄게 그린다. 썸네일로 잡히는 자리이기도 하다.
   *
   * 주제 + 범위가 다 들어가야 한다. 예: "한국 부자 TOP 20", "공기업 평균연봉 TOP 16"
   * (나쁜 예: "연봉 이야기", "알아두면 좋은 정보")
   */
  headline?: string;
  /** 화면 큰 텍스트. \n 으로 줄바꿈, 최대 3줄 */
  title: string;
