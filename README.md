...
**유튜브 바로 올리기** — **예약 발행**(기본) / 바로 발행 / 비공개로만 올리기 중에 고릅니다.
...
올릴 때는 **영상을 먼저 비공개로 올리고, 그다음에 공개·예약을 겁니다.**
설정이 거절돼도 올린 영상이 날아가지 않게 하려는 것입니다
(예전에는 한 번에 보내서, 예약이 거절되면 다 올린 영상까지 통째로 사라졌습니다).
공개·예약만 실패하면 **영상 링크와 함께** 무엇이 막혔는지 알려주니,
유튜브 스튜디오에서 마저 바꾸면 됩니다.

> 공개 설정을 걸려면 `youtube.force-ssl`(계정 관리) 권한이 필요합니다.
> **이 기능이 생기기 전에 계정을 연결했다면 [계정 다시 연결]** 을 한 번 눌러주세요.
> 권한이 없으면 영상은 비공개로 올라가고 예약만 실패하며, 화면에 미리 알려줍니다.
...
예전에는 비공개는 올라가는데 **예약·바로 공개만** 이 오류로 죽는 일이 있었습니다.
공개 설정을 업로드 본문과 같은 요청에 실었기 때문인데, 지금은 분리해서
**영상이 먼저 올라간 뒤** 공개 설정을 겁니다. 설정이 막히더라도 영상은 남습니다.
